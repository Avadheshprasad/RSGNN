...
def feature_flip(number_flip, x):
    ...
    np.random.seed(42)
    if number_flip > 0:
        total_elements = x1.shape[0] * x1.shape[1]
        flip_indices = np.random.choice(total_elements, size=int(number_flip), replace=False)
        row_indices, col_indices = np.unravel_index(flip_indices, x1.shape)
        idex_fea = x1[ row_indices, col_indices]
        x1[row_indices, col_indices] = np.where(idex_fea == 0, 1, 0)
